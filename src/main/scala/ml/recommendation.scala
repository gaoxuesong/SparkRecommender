...
object recommendation {
  ...
  def main(args: Array[String]) {
    println("Spark ALS for recommendation begin...")
    println("Version 0.0.2 6/26/2014")
    ...
    val conf = new SparkConf()
      .setMaster("local")//.setMaster("spark://hadoop140.platform:7077")
      .setAppName("Recommender")
      .set("spark.executor.memory", "8g")
    //      .setSparkHome("/opt/cloudera/parcels/CDH-5.0.0-1.cdh5.0.0.p0.47/lib/spark")
    //      .setJars(Seq("/opt/SparkALS/bin/SparkALSApp.jar"))
    val sc = new SparkContext(conf)

    // load ratings and movie titles

    // val movieLensHomeDir = "/opt/movies/medium"
    val movieLensHomeDir = "./movies"

    // E:\data\ml-1m
    //val ratings = sc.textFile(movieLensHomeDir + "/ratings.dat").map { line =>
    val ratings = sc.textFile("C:\\GitHub\\SparkAlgorithmsApp\\data\\ml-1m" + "\\ratings.dat").map { line =>
    ...
    val movies = sc.textFile("C:\\GitHub\\SparkAlgorithmsApp\\data\\ml-1m" + "/movies.dat").map { line =>
      val fields = line.split("::")
